...
predict.genes.in.class <- function(path = ".", tissue.mat, prediction.mat = NULL, 
trial.num = NULL, vote.min = 0.5, verbose = FALSE){
	...
	if(!is.null(trial.num)){
		...
		for(n in 1:length(trial.num)){
			if(verbose){report.progress(n, length(trial.num))}
