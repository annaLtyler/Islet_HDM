...
write.data.for.shiny <- function(data.obj, geno.obj, name = "myData", path = "."){
		...
		shiny.obj$raw.pheno <- data.obj$raw_pheno
		...
		geno.locale <- match(all.markers, dimnames(geno.obj)[[3]])
		...
		shiny.obj$marker.location <- data.obj$marker_location[marker.locale]
		shiny.obj$marker.num <- data.obj$marker_num[marker.locale]
		shiny.obj$geno.names <- data.obj$geno_names
		shiny.obj$geno.names[[3]] <- data.obj$geno_names[[3]][marker.locale]
		shiny.obj$var.to.var.p.val <- data.obj$var_to_var_p_val
		shiny.obj$max.var.to.pheno.influence <- data.obj$max_var_to_pheno_influence
		shiny.obj$p.covar <- data.obj$p_covar
		shiny.obj$p.covar.table <- data.obj$p_covar_table
		shiny.obj$g.covar <- data.obj$g_covar
		shiny.obj$g.covar.table <- data.obj$g_covar_table
