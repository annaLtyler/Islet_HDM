...
if(!file.exists(data.results.dir)){dir.create(data.results.dir, recursive = TRUE)}
...
write.table(pheno.data$annot.phenotype, file.path(data.results.dir, "Phenotype_Annotations.txt"), 
...
plot.with.model(weight[not.outlier], glu[not.outlier], xlab = "Weight (g)",
  ylab = "Fasting Glucose (mg/dL)", col = sex+2)
...
plot.with.model(num.pheno[,"food_ave"], num.pheno[,"weight_final"],
  ylab = "Weight (g)",
  xlab = "Avg Food Consumption (AU)", col = sex+2)
...
plot.with.model(num.pheno[,"Glu_tAUC"], num.pheno[,"weight_final"],
...
```{r herit, fig.width = 7, fig.height = 5}
library(qtl2)
...
saveRDS(num.covar, file.path(data.results.dir, "Clinical_Phenotype_Covariates.RDS"))


herit  <- est_herit(norm.traits, Kg, addcovar = num.covar)
ylim <- get_plot_bounds(min.val = 0, max.val = (max(as.numeric(herit))), 
...
saveRDS(scaled.pheno, file.path(data.results.dir, "Clinical_Phenotypes_Adjusted.RDS"))
```
...
```{r final_herit, fig.width = 5, fig.height = 4}
herit  <- est_herit(scaled.pheno, Kg, addcovar = num.covar)
ylim <- get_plot_bounds(min.val = 0, max.val = (max(as.numeric(herit))), 
  scale.factor = 10, return.even = TRUE)
herit.order <- order(as.numeric(herit))
par(mar = c(8, 4, 4, 2))
barplot(as.numeric(herit)[herit.order], names = colnames(scaled.pheno)[herit.order], las = 2,
  ylab = "Heritability", ylim = ylim)
abline(h = seq(0, ylim[2], 0.1), lty = 2, col = "darkgray")
```