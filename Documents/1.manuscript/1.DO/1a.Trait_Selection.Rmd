...
#horizontal
pdf(here("Results", "Paper", "Fig1F.pdf"), width = 10, height = 5)
...
pheatmap(genetic.cor[geno.cor.order, geno.cor.order], cluster_rows = FALSE, cluster_cols = FALSE)
pheatmap(genetic.cor[geno.cor.order, pheno.cor.order], cluster_rows = FALSE, cluster_cols = FALSE)
...
colnames(combo.mat) <- colnames(cor.class) <- colnames(genetic.cor)[geno.cor.order]
rownames(combo.mat) <- rownames(cor.class) <- rownames(genetic.cor)[geno.cor.order]
...
for(i in 1:nrow(trait.pairs)){
  ...
  upper.tri.idx <- get_tri_idx(genetic.cor, trait1, trait2, "upper")
  lower.tri.idx <- get_tri_idx(genetic.cor, trait1, trait2, "lower")

  combo.mat[upper.tri.idx] <- genetic.cor[trait1, trait2] #put the genetic cor in the upper triangle
  combo.mat[lower.tri.idx] <- pheno.cor[trait1, trait2] #put the phenotype cor in the lower triangle
...
cor.class.cols <- c("red", "blue", "yellow")

png(here("Results", "Paper", "Fig1G.png"), width = 6.5, height = 5, units = "in", res = 300)
layout.mat <- matrix(c(1,2,1,3,1,4), byrow = TRUE, ncol = 2)
layout(layout.mat, widths = c(1, 0.15))
par(mar = c(6,1,1,8))
...
mtext("Phenotype\nCorrelation", side = 3, cex = 0.7, line = 0.8)
par(mar = c(1,2,4,3))
imageWithTextColorbar(matrix(as.numeric(herit), ncol = 1), 
