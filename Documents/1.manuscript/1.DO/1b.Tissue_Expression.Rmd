...
for(tx in 1:length(local.eQTL.pos[[i]])){
  ...
  if(plot.type == "points"){
    ...
    plot.with.model(mean.local, mean.distal, 
      ...
      xlim = c(0.2, 1.1), ylim = c(0.1, 0.4))

    se.local <- apply(local.sqrd, 2, function(x) sd(x, na.rm = TRUE)/sqrt(length(which(!is.na(x)))))
    se.distal <- apply(distal.sqrd, 2, function(x) sd(x, na.rm = TRUE)/sqrt(length(which(!is.na(x)))))
...
plot.new()
plot.window(xlim = c(0,1), ylim = c(0,1))
legend(0, 1, fill = qtl2::CCcolors, legend = names(qtl2::CCcolors), cex = 1.5)

plot.new()
plot.window(xlim = c(0,1), ylim = c(0,1))
legend(0, 1, pch = subsp.pch, legend = names(subsp), cex = 1.5)
```
