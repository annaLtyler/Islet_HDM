...
five tissues: adipose, islet, liver, and skeletal 
muscle.
...
if(!file.exists(parsed.data.dir)){dir.create(parsed.data.dir)}
```
...
saveRDS(gene.tables, file.path(parsed.data.dir, "Gene_Tables.RDS"))
```
...
if(remove.pseudogenes){
  description.files <- here("Data", "general", paste0("Gene_Description_", tissue.names, ".RDS"))
  ...
  for(tx in 1:length(tissue.data)){
    ...
    riken.idx <- grep("riken", one.gene.info[,"description"], ignore.case = TRUE)
    remove.idx <- Reduce("union", list(pred.idx, pseudo.idx, riken.idx))
...
```{r adjust}
adj.expr.file <- file.path(parsed.data.dir, "Tissue_Expression_Adjusted.RDS")
...
#pdf("~/Desktop/transcript_pc.pdf", width = 7, height = 5)
par(mar = c(4,4,4,1))
...
abline(v = seq(-0.025, 0.035, 0.01), col = "lightgray", lwd = 0.5, lty = "dotted")
points(expr.decomp$u, col = "black", pch = sample.sex+1)
points(expr.decomp$u, col = tissue.col, pch = sample.sex+16)
par(mar = c(4,0,4,0))
plot.new()
plot.window(xlim = c(0,1), ylim = c(0,1))
legend(0, 1, legend = tissue.names, col = "black", pch = 1, adj = 0, title = "Tissue")
...
```{r supp_fig2}
pdf(here("Results", "Paper", "Supp_Fig2A.pdf"), width = 7, height = 5)
par(mar = c(4,4,4,1))
layout(matrix(c(1,2), nrow = 1), widths = c(1, 0.3))
plot(expr.decomp$u,
  xlab = paste0("PC1 (", round(expr.decomp$var.exp[1]*100), "%)"),
...
pc.table <- data.frame("Tissue" = names(tissue.col), "Sex" = sample.sex,
  "PC1" = expr.decomp$u[,1], "PC2" = expr.decomp$u[,2])
write_table_with_header(pc.table, file.path(figure.data.dir, "Supp_Fig2A.txt"),
  header = "Data for Supplemental Figure 2A. The first two principal 
  components of the gene expression matrix across all samples.
  The sex of each animal is indicated as 0 = female and 1 = male.")
```
...
```{r supp_fig2B_E}
pdf(here("Results", "Paper", "Supp_Fig2_B-E.pdf"), width = 7, height = 7)
for(tx in 1:length(tissue.data)){
  cat("###", tissue.names[tx], "\n")
  tissue.eqtl <- tissue.data[[tx]]$lod.peaks$additive
  tissue.eqtl <- tissue.eqtl[which(tissue.eqtl[,"lod"] >= min.lod),]
...
for(tx in 1:length(tissue.data)){
  ...
  write_table_with_header(data.table = eqtl.table, 
  file.name = file.path(figure.data.dir, paste0("Supp_Fig2", LETTERS[(tx+1)], ".txt")), 
    header = paste0("Data for Supplemental Figure 2", LETTERS[(tx+1)], 
    ". eQTL data for ", tissue.names[tx]))
}
...
par(mar = c(4,4,4,1))
barplot(eqtl.count.table[c("Local", "Distal"),], col = herit.cols[c("Local", "Distal")],
  ylab = "eQTL Count", cex.names = 1.5)
par(mar = c(4,0,4,1))
plot.new()
plot.window(xlim = c(0,1), ylim = c(0,1))
legend(x = 0, y = 0.5, legend = c("Distal", "Local"), fill = herit.cols[c("Distal", "Local")])
```

```{r supp_fig2F}

pdf(here("Results", "Paper", "Supp_Fig2F.pdf"), width = 7, height = 5)
layout(matrix(c(1,2), nrow = 1), widths = c(1,0.2))
par(mar = c(4,4,4,1))
barplot(eqtl.count.table[c("Local", "Distal"),], col = herit.cols[c("Local", "Distal")],
  ylab = "eQTL Count", cex.names = 1.2)
par(mar = c(4,0,4,1))
plot.new()
plot.window(xlim = c(0,1), ylim = c(0,1))
legend(x = 0, y = 0.5, legend = c("Distal", "Local"), fill = herit.cols[c("Distal", "Local")])
...
fig.table <- cbind(rownames(eqtl.count.table[c("Local", "Distal"),]), eqtl.count.table[c("Local", "Distal"),])
colnames(fig.table)[1] <- "eQTL_type"
write_table_with_header(fig.table, file.path(figure.data.dir, "Supp_Fig2F.txt"),
 header = "Data for Supplemental Figure 2F. Counts of local and distal
 eQTL across tissues at an FDR of 0.01.")
```
...
```{r supp_fig2G}
png(here("Results", "Paper", "Supp_Fig2G.png"), width = 8, height = 5, units = "in", res = 300)
#pdf(here("Results", "Paper", "Supp_Fig2G.pdf"), width = 8, height = 5)
par(bg = NA)
bar.col <- rep("gray", nrow(combo.mat))
single.tissue <- which(rowSums(combo.mat) == 1)
bar.col[single.tissue] <- rev(tissue.cols)
...
text(x = a[single.tissue], y = combo.counts[single.tissue]+100, 
  labels = rev(tissue.names), srt = 90, adj = 0)
par(mar = c(1,6,0,4))
imageWithText(t(combo.mat), col.scale = "blue", row.text.shift = 0.04)
mtext("Shared Local eQTL", side = 3, outer = TRUE, line = -1.5)
dev.off()


fig.table <- cbind(combo.mat, combo.counts)
colnames(fig.table)[5] <- "eQTL Count"
write_table_with_header(fig.table, file.path(figure.data.dir, "Supp_Fig2G.txt"),
  header = "Data for Supplemental Figure 2G. Each row identifies which tissues
  are included in the count. The final column indicates the number of
  shared local eQTL across the tissues indicated.")
```
...
```{r supp_fig2H}
png(here("Results", "Paper", "Supp_Fig2H.png"), width = 8, height = 5, units = "in", res = 300)
#pdf(here("Results", "Paper", "Supp_Fig2H.pdf"), width = 8, height = 5)
par(bg = NA)
bar.col <- rep("gray", nrow(combo.mat))
single.tissue <- which(rowSums(combo.mat) == 1)
bar.col[single.tissue] <- rev(tissue.cols)
...
fig.table <- cbind(combo.mat, combo.counts)
colnames(fig.table)[5] <- "eQTL Count"
write_table_with_header(fig.table, file.path(figure.data.dir, "Supp_Fig2H.txt"),
  header = "Data for Supplemental Figure 2H. Each row identifies which 
  tissues are included in the count. The final column indicates the 
  number of shared distal eQTL across the tissues indicated.")

```

## Subspecies Effects {.tabset .tabset-fade .tabset-pills}

We looked at whether local and distal eQTLs had varying contributions
to heritability across the subspecies (domesticus, musculus, and castaneus).
We looked at eQTLs across several LOD score thresholds ranging from 8 to
12.

At each threshold we collected the haplotype coefficients for all local
and distal eQTLs. This resulted in two matrices. In each the rows represented
eQTLs, either local or distal, and the columns were haplotypes.
We compared the mean of the squares of the coefficients across the haplotypes.

The castaneus and musculus subspecies tended to have large local 
coefficients, as has been shown multiple times previously. Interestingly,
the distal coefficients were relatively constant across the three 
subspecies.


We calculated the relative contribution of local haplotype ($R_L$) 
as follows:

1. Calculate the mean squared local coefficient ($L$) for each strain ($S$): ($L_S^2$)
2. Calculate the mean squared distal coefficient ($D$) for each strain ($S$): ($D_S^2$)
3. Calculate the relative strength of the local coefficients ($L_R$): ($L_R = \frac{L_S^2}{L_S^2+D_S^2}$)

The local haplotype tended to have a relatively larger effect in the
castaneus and musculus subspecies ($R_L > 0.5$) than in the domesticus 
subspecies ($R_L < 0.5$). This may be because there are many more local
variants affecting transcription in the two more distant subspecies, which
translates into high local heritability. However, the effects of distal
variants are buffered by the transcriptional network and thus have a 
relatively constant effect across subspecies.


```{r, subspecies_effects, fig.width = 10, fig.height = 7, results = "asis"}
subsp <- list("domesticus" = c("A", "B", "C", "D", "E", "H"), 
  "castaneus" = "F", "musculus" = "G")
#subsp.col <- c("domesticus" = "#7fc97f", "castaneus" = "#beaed4", "musculus" = "#fdc086")
subsp.col <- c("domesticus" = "gray90", 
  "castaneus" = as.vector(CCcolors["CAST"]), 
  "musculus" = as.vector(CCcolors["PWK"]))
subsp.pch <- c(16, 3, 11)
sub.pch <- sub.col <- rep(NA, 8)

for(i in 1:length(subsp)){
  sub.pch[which(LETTERS %in% subsp[[i]])] <- subsp.pch[i]
  sub.col[which(LETTERS %in% subsp[[i]])] <- subsp.col[i]
}

plot.type = "boxes"
#plot.type = "points"

extract_coef <- function(tx.eqtl, lodthresh = 8){
  if(nrow(tx.eqtl) == 0){
    return(rep(NA, 8))
  }
  pass.thresh <- which(tx.eqtl$lod >= lodthresh)
  if(length(pass.thresh) == 0){
    return(rep(NA, 8))
  }
  return(as.matrix(tx.eqtl[pass.thresh,4:11]))
}

plot_rel <- function(local.distal.list, plot.type = c("local", "distal"), title = ""){
    
    plot.type = plot.type[1]
    mean.local.hap <- colMeans(local.distal.list$local, na.rm = TRUE)
    mean.distal.hap <- colMeans(local.distal.list$distal, na.rm = TRUE)
    total.hap <- colMeans(rbind(local.distal.list$local, local.distal.list$distal), na.rm = TRUE)
    #total.hap <- mean.distal.hap+mean.local.hap #gives outsized weight to distal component

    rel.distal <- mean.distal.hap/total.hap
    rel.local <- mean.local.hap/total.hap

    if(plot.type == "local"){
      plot.this <- rel.local
    }else{
      plot.this <- rel.distal
    }

    layout(matrix(c(3,3,3,1,2,0), nrow = 2, byrow = TRUE), widths = c(1, 1, 0.2), heights = c(1, 0.7))

    good.ylim <- get_plot_bounds(0, max(plot.this, na.rm = TRUE), scale.factor = 10)

    par(mar = c(6,6,0,0))
    a <- barplot(plot.this, beside = TRUE, col = sub.col, 
      ylab = paste(local.or.distal, "contribution"), ylim = good.ylim,
      cex.lab = 1.5, cex.names = 1.5, las = 2, cex.axis = 1.5)
    plot.dim <- par("usr")
    segments(x0 = plot.dim[1], x1 = plot.dim[2], y0 = 1)

    par(mar = c(4,4,4,2))
    plot.new()
    plot.window(xlim = c(0,1), ylim = c(0,1))
    legend(0, 1, fill = subsp.col, legend = names(subsp), cex = 1.5)

    par(mar = c(6,4,4,4))
    plot.grouped.boxes(local.distal.list, type = "matrix",
      main = "", ylab = "eQTL Coef. Squared", 
      group.cols = c("local" = local.col, "distal" = distal.col), print.vals = NA)
    
    plot.dim <- par("usr")
    plot.height <- plot.dim[4] - plot.dim[3]
    box.x <- seq((plot.dim[1]+1.2), (plot.dim[2]-1.5), length.out = 9)
    box.min.y <- plot.dim[3] - (plot.height*0.05)
    box.max.y <- plot.dim[3] - (plot.height*0.02)
    for(i in 1:length(box.x)){
      draw.rectangle(min.x = box.x[i], max.x = box.x[(i+1)], min.y = box.min.y, max.y = box.max.y, fill = sub.col[i])
    }

    mtext(title, side = 3, line = -2.5, outer = TRUE, font = 2)
    invisible(plot.this)
}


lodthresh.seq = seq(8, 12)
local.or.distal <- "local"

#lodthresh.seq = seq(8, 52, 4)
rel.val.list <- num.val.list <- vector(mode = "list", length = length(lodthresh.seq))

for(l in 1:length(lodthresh.seq)){
  #pdf(paste0("~/Desktop/relative_subspecies_contributions_", lodthresh.seq[l], ".pdf"), width = 10, height = 8)
  #layout(matrix(c(1,2,5,3,4,6), nrow = 2, byrow = TRUE))
  cat("### LOD:", lodthresh.seq[l], "{.tabset .tabset-fade .tabset-pills}\n")
  rel.val.mat <- matrix(NA, ncol = 8, nrow = length(tissue.names))
  colnames(rel.val.mat) <- names(CCcolors)
  rownames(rel.val.mat) <- tissue.names
  num.eqtl.mat <- matrix(NA, ncol = 2, nrow = length(tissue.names))
  colnames(num.eqtl.mat) <- c("local", "distal")
  rownames(num.eqtl.mat) <- tissue.names
  for(tx in 1:length(local.eQTL.pos[[i]])){
    cat("####", tissue.names[tx], "\n")

    local.distal.file <- file.path(results.dir, 
      paste0("Local.Distal.Coef.Sqrd.", tissue.names[tx], ".LOD", lodthresh.seq[l], ".RDS"))
    if(!file.exists(local.distal.file)){

      rownames(rel.val.mat) <- tissue.names
      colnames(rel.val.mat) <- names(CCcolors)

      local.tx.eQTL <- t(sapply(local.eQTL.pos, function(x) extract_coef(x[[tx]], lodthresh = lodthresh.seq[l])))
      distal.tx.eQTL <- Reduce("rbind", lapply(distal.eQTL.pos, function(x) extract_coef(x[[tx]], lodthresh = lodthresh.seq[l])))
      colnames(local.tx.eQTL) <- colnames(distal.tx.eQTL) <- names(CCcolors)

      #local.sqrd <- local.tx.eQTL^2
      #distal.sqrd <- distal.tx.eQTL^2

      local.sqrd <- abs(local.tx.eQTL)
      distal.sqrd <- abs(distal.tx.eQTL)

      local.distal.sqrd <- list("local" = local.sqrd, "distal" = distal.sqrd)
      saveRDS(local.distal.sqrd, local.distal.file)
    }else{
      local.distal.sqrd <- readRDS(local.distal.file)
    }
    num.local.distal <- sapply(local.distal.sqrd, function(x) length(which(!is.na(rowSums(x)))))
    num.eqtl.mat[tx,] <- num.local.distal
    rel.vals <- plot_rel(local.distal.sqrd, plot.type = local.or.distal, title = tissue.names[tx])
    rel.val.mat[tx,] <- rel.vals
    cat("\n\n")
  }
  rel.val.list[[l]] <- rel.val.mat
  num.val.list[[l]] <- num.eqtl.mat
  cat("\n\n")
} #end looping through lodthresh.seq
```
 

```{r supp_fig_local_distal, eval = FALSE}
l = 1
tx = 1

pdf(here("Results", "Paper", "Supp_Fig_Distal_Comparison1.pdf"), width = 10, height = 6)
local.distal.file <- file.path(results.dir, 
  paste0("Local.Distal.Coef.Sqrd.", tissue.names[tx], ".LOD", lodthresh.seq[l], ".RDS"))
local.distal.sqrd <- readRDS(local.distal.file)
plot_rel(local.distal.sqrd, plot.type = "distal", title = "Adipose, LOD:8")
dev.off()

pdf(here("Results", "Paper", "Supp_Fig_Local_Comparison1.pdf"), width = 10, height = 6)
local.distal.file <- file.path(results.dir, 
  paste0("Local.Distal.Coef.Sqrd.", tissue.names[tx], ".LOD", lodthresh.seq[l], ".RDS"))
local.distal.sqrd <- readRDS(local.distal.file)
plot_rel(local.distal.sqrd, plot.type = "local", title = "Adipose, LOD:8")
dev.off()
```
...
test_diff <- function(coef.mat, plot.label = ""){
  test.diff <- aov.by.matrix(coef.mat, return.which = "aov")
  diff.tukey <- TukeyHSD(test.diff)
  neg.p <- -log10(diff.tukey$fact[,"p adj"]+1e-6)
  neg.p.mat <- matrix(NA, nrow = 8, ncol = 8)
  rownames(neg.p.mat) <- colnames(neg.p.mat) <- colnames(coef.mat)
  for(i in 1:length(neg.p)){
    split.name <- strsplit(names(neg.p)[i], "-")
    row.num <- as.numeric(split.name[[1]][1])
    col.num <- as.numeric(split.name[[1]][2])
    neg.p.mat[row.num,col.num] <- neg.p[i]
    neg.p.mat[col.num,row.num] <- neg.p[i]
  }
  imageWithText(signif(neg.p.mat, 2), use.pheatmap.colors = TRUE, cex = 0.8,
    row.text.shift = 0.08, col.text.shift = 0.1, col.text.rotation = 0, col.text.adj = 0.5,
    main = plot.label)
}



png(here("Results", "Paper", "Reviewer_Response_Subspecies.png"), width = 10, height = 10, units = "in", res = 300)
par(mfrow = c(2,2), mar = c(4,4,4,4))
boxplot(local.distal.sqrd$local, col = sub.col, main = "Local Coefficients", ylab = "|eQTL Coefficients|", las = 2)
legend("topleft", fill = subsp.col, legend = names(subsp.col))
test_diff(local.distal.sqrd$local, plot.label = "-log10 (p+1e-6) for Local Differences Tukey's HSD")
par(mar = c(4,4,4,4))
boxplot(local.distal.sqrd$distal, col = sub.col, main = "Distal Coefficients", ylab = "|eQTL Coefficients|", las = 2)
legend("topleft", fill = subsp.col, legend = names(subsp.col))
test_diff(local.distal.sqrd$distal, plot.label = "-log10 (p+1e-6) for Distal Differences Tukey's HSD")
dev.off()
...
We collected the mean contributions across all LOD thresholds
and combined them in the following figures. 

The bar plot shows how many local and distal eQTLs there are
at each threshold.

```{r num_eqtl}
num.local.eqtl <- sapply(num.val.list, function(x) x[,1])
num.distal.eqtl <- sapply(num.val.list, function(x) x[,2])
colnames(num.local.eqtl) <- colnames(num.distal.eqtl) <- lodthresh.seq

barplot(num.local.eqtl, beside = TRUE, col = tissue.cols, ylab = "Num eQTL", main = "Number of Local eQTL", xlab = "LOD threshold")
abline(h = seq(0, 10000, 1000), lty = 2, col = "darkgray")
legend("topright", fill = tissue.cols, legend = tissue.names, bg = "white")

barplot(num.distal.eqtl, beside = TRUE, col = tissue.cols, ylab = "Num eQTL", main = "Number of Distal eQTL", xlab = "LOD threshold", ylim = c(0, 2000))
abline(h = seq(0, 2000, 250), lty = 2, col = "darkgray")

perc.distal <- num.distal.eqtl*100/(num.distal.eqtl+num.local.eqtl)
barplot(perc.distal, beside = TRUE, col = tissue.cols, ylab = "Percent Distal eQTL", 
  main = "Percent Distal eQTL", xlab = "LOD threshold", ylim = c(0, 15))
abline(h = seq(0, 14, 2), lty = 2, col = "darkgray")
```

Each box shows the
mean values across the tissues for that threshold.


```{r susp_collected, fig.width = 12, fig.height = 5}
lod.cols <- colorRampPalette(c("white", "blue"))(length(lodthresh.seq))
names(rel.val.list) <- lodthresh.seq
plot.grouped.boxes(rel.val.list, type = "matrix", print.vals = NA, legend.horiz = TRUE,
  ylab = "Relative Contribution to Local eQTL", group.cols = lod.cols, 
  legend.title = "LOD threshold")
plot.dim <- par("usr")
plot.height <- plot.dim[4] - plot.dim[3]
plot.length <- plot.dim[2] - plot.dim[1]
segments(x0 = plot.dim[1], x1 = plot.dim[2], y0 = 1)
x.seq <- seq((plot.dim[1]+(plot.length*0.04)), (plot.dim[2] - (plot.length*0.04)), length.out = 9)
for(i in 1:(length(x.seq)-1)){
  #text(x = mean(c(x.seq[i], x.seq[(i+1)])), y = 0.84, labels = names(CCcolors)[i])
  draw.rectangle(min.x = x.seq[i], max.x = x.seq[(i+1)], 
    min.y = (plot.dim[3]-(plot.height*0.06)), max.y = (plot.dim[3]-(plot.height*0.03)), fill = sub.col[i])
}
legend("topright", fill = subsp.col, legend = names(subsp.col), title = "Susbpscies")
#dev.off()
```

```{r lod_thresh, eval = FALSE}
pdf(here("Results", "Paper", paste0("Supp_Fig_", local.or.distal, "_Comparison2.pdf")), width = 10, height = 5)
lod.cols <- colorRampPalette(c("white", "blue"))(length(lodthresh.seq))
names(rel.val.list) <- lodthresh.seq
plot.grouped.boxes(rel.val.list, type = "matrix", print.vals = NA, legend.horiz = TRUE,
  ylab = "Relative Contribution to Distal eQTL", group.cols = lod.cols, 
  legend.title = "LOD threshold")
plot.dim <- par("usr")
plot.height <- plot.dim[4] - plot.dim[3]
plot.length <- plot.dim[2] - plot.dim[1]
...
```{r dots, eval = FALSE}
#maybe better as a dot plot, so we can see all the data?
#plot each tissue separately
pdf("~/Desktop/Tissues_separate.pdf", width = 9, height = 5)
for(tx in 1:length(tissue.names)){
  tissue.mat <- sapply(rel.val.list, function(x) x[tx,])
  plot.new()
  plot.window(xlim = c(1,ncol(tissue.mat)), ylim = c(0.3, 0.65))
  for(st in 1:nrow(tissue.mat)){
    #points(x = 1:ncol(tissue.mat), y = tissue.mat[st,], col = CCcolors[st], lwd = 3, type = "b")
    points(x = 1:ncol(tissue.mat), y = tissue.mat[st,], col = sub.col[st], lwd = 3, type = "b")
  }
  axis(1, at = 1:ncol(tissue.mat), labels = lodthresh.seq)
  axis(2)
  mtext(tissue.names[tx], side = 3)
  mtext("LOD threshold", side = 1, line = 2.5)
  mtext("Relative Contribution to Local eQTL", side = 2, line = 2.5)
  abline(h = 0.5)
  legend("topright", fill = subsp.col, legend = names(subsp.col))
}
dev.off()
```
...
#and thresholding
local.lod.file <- here("Results", "DO", "Transcriptomes", "Local_LOD.RDS")
...
```{r, eval = FALSE}
clin.pheno <- readRDS(file.path(parsed.data.dir, "Clinical_Phenotypes_Adjusted.RDS"))
...
```{r network_degree, eval = FALSE}
gene.deg.file <- file.path(results.dir, "Transcript_Degree.RDS")
