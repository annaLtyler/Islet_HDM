...
rel.local.list <- vector(mode = "list", length = length(lodthresh.seq))
for(l in 1:length(lodthresh.seq)){
  ...
  layout(matrix(c(1,2,3,4,5,5,5,5), nrow = 2, byrow = TRUE), widths = c(1, 0.2, 1, 0.2))
  rel.local.mat <- matrix(NA, ncol = 8, nrow = length(tissue.names))

  for(tx in 1:length(local.eQTL.pos[[i]])){
    rownames(rel.local.mat) <- tissue.names
    colnames(rel.local.mat) <- LETTERS[1:8]
    ...
    local.std <- t(apply(local.tx.eQTL, 1, function(x) x/sd(x)))
    distal.std <- t(apply(distal.tx.eQTL, 1, function(x) x/sd(x)))

    local.sqrd <- local.std^2
    distal.sqrd <- distal.std^2

    mean.local.hap <- colMeans(local.sqrd, na.rm = TRUE)
    mean.distal.hap <- colMeans(distal.sqrd, na.rm = TRUE)

    rel.distal <- mean.distal.hap/(mean.local.hap+mean.distal.hap)
    rel.local <- mean.local.hap/(mean.local.hap+mean.distal.hap)
    rel.local.mat[tx,] <- rel.local

    #barplot(rel.distal, beside = TRUE, col = CCcolors, ylab = "Relative Cotribution of Distal Effects")
    #abline(h = 0.5)
    #barplot(rel.distal, beside = TRUE, col = sub.col, ylab = "Relative Cotribution of Distal Effects")
    #abline(h = 0.5)

    par(mar = c(4,4,4,0))
    barplot(rel.local, beside = TRUE, col = CCcolors, ylab = "Relative Cotribution of Local Effects")
    abline(h = 0.5)
    par(mar = c(4,0,4,2))
    plot.new()
    plot.window(xlim = c(0,1), ylim = c(0,1))
    legend(0, 1, fill = qtl2::CCcolors, legend = names(qtl2::CCcolors))

    par(mar = c(4,4,4,0))
    barplot(rel.local, beside = TRUE, col = sub.col, ylab = "Relative Cotribution of Local Effects")
    abline(h = 0.5)
    par(mar = c(4,0,4,2))
    plot.new()
    plot.window(xlim = c(0,1), ylim = c(0,1))
    legend(0, 1, fill = subsp.col, legend = names(subsp))


    if(plot.type == "boxes"){
      #boxplot(local.sqrd, names = LETTERS[1:8], col = qtl2::CCcolors)
      #boxplot(distal.sqrd, names = LETTERS[1:8], col = qtl2::CCcolors)
      #plot.grouped.boxes(list("local" = local.sqrd, "distal" = distal.sqrd), type = "matrix")
      par(mar = c(4,4,4,4))
      plot.grouped.boxes(list("local" = abs(local.tx.eQTL), "distal" = abs(distal.tx.eQTL)), type = "matrix",
        main = tissue.names[tx], ylab = "eQTL Effect Size")
    
    }

    if(plot.type == "points"){
      mean.local <- colMeans(local.sqrd, na.rm = TRUE)
      mean.distal <- colMeans(distal.sqrd, na.rm = TRUE)
      plot.with.model(mean.local, mean.distal, 
        col = qtl2::CCcolors, pch = sub.pch, xlab = "Mean Local Haplotype^2", 
        ylab = "Mean Distal Haplotype^2", main = tissue.names[tx], cex = 2)

      #se.local <- apply(local.sqrd, 2, function(x) sd(x, na.rm = TRUE))
      #se.distal <- apply(distal.sqrd, 2, function(x) sd(x, na.rm = TRUE))
      #segments(x0 = mean.local, y0 = mean.distal - se.distal,
      #  y1 = mean.distal + se.distal, col = qtl2::CCcolors)
      #segments(x0 = mean.local - se.local, x1 = mean.local + se.local, y0 = mean.distal, 
      #  col = CCcolors, lwd = 3)
    }

    mtext(tissue.names[tx], side = 3, line = -2.5, outer = TRUE, font = 2)
  }
  dev.off()
  rel.local.list[[l]] <- rel.local.mat
} #end looping through lodthresh.seq


lod.cols <- colorRampPalette(c("white", "blue"))(length(lodthresh.seq))
#barplot(1:length(gray.scale), col = gray.scale)
pdf("~/Desktop/local_comparison_boxes.pdf", width = 12, height = 5)
names(rel.local.list) <- lodthresh.seq
plot.grouped.boxes(rel.local.list, type = "matrix", print.vals = NA, legend.horiz = TRUE,
  ylab = "Relative Contribution to Local eQTL", group.cols = lod.cols, 
  legend.title = "LOD threshold")
plot.dim <- par("usr")
segments(x0 = plot.dim[1], x1 = plot.dim[2], y0 = 0.5)
x.seq <- seq(plot.dim[4], plot.dim[2], 9)
for(i in 1:(length(x.seq)-1)){
  draw.rectangle(min.x = x.seq[i], max.x = x.seq[(i+1)], min.y = 0.24, max.y = plot.dim[3], fill = CCcolors[i])
  draw.rectangle(min.x = x.seq[i], max.x = x.seq[(i+1)], min.y = 0.18, max.y = 0.208, fill = sub.col[i])
}
dev.off()
...
for(tx in 1:length(tissue.names)){
  tissue.mat <- sapply(rel.local.list, function(x) x[tx,])
  plot.new()
