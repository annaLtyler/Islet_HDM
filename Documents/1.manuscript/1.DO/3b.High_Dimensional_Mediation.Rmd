...
pdf("~/Desktop/latent_cor.pdf", width = 9, height = 6)
layout.mat <- matrix(c(0,1:4,5,8,11,14,17,6,9,12,15,18,7,10,13,16,19), 
    ncol = 5, byrow = TRUE)
layout(layout.mat, widths = c(0.15, rep(1, 4)), 
    heights = c(0.2, rep(1, 3), 0.2))
#add tissue headers
for(tx in 1:length(tissue.names)){
    par(mar = c(0,4,0,0))
    plot.text(tissue.names[tx], cex = 2, font = 2, adj = 0.5)
}
#add plot labels
...
for(l in 1:length(plot.labels)){
    par(mar = c(4,0,0,0))
    plot.text(plot.labels[l], x = 0.7 , y = 0.5, cex = 2, font = 2, srt = 90, adj = 0.5)
}
...
for(tx in 1:length(tissue.names)){
    ...
    if(complete.mediation){
        ...
        axis.labels <- c("Cmpst. Genome", "Cmpst. Transcriptome", "Cmpst. Phenome")
    }else{
        colnames(curr_scores) <- c("Genome", "Phenotype", "Transcriptome")
        axis.labels <- c("Cmpst. Genome", "Cmpst. Phenome", "Cmpst. Transcriptome")
    }
