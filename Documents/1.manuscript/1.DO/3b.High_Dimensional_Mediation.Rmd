...
pheno.model.cor <- lapply(model_scores, function(x) apply(pheno.pc, 2, function(y) cor(x[,"Outcome"], y)))
model.var.exp <- lapply(pheno.model.cor, function(x) sum(x*pc.var.exp))

saveRDS(lapply(model.var.exp, function(x) x*100), here("Results", "Paper", "Model_Variance_Explained.RDS"))
```
...
for(tx in 1:max.tx){
 if(max.tx == 1){
    ...
    saveRDS(tx.pcor, here("Results", "Paper", "Pcor.RDS"))
}
```
