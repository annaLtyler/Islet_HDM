...
if(!is.null(adjust_before_fitting)){
    adjust.file <- here("Results", "High_Dim_Med_no_MGE", exp.name, adjust_before_fitting)
...
results.dir <- here("Results", "High_Dim_Med_no_MGE", exp.name)
...
```{r pheno}
scaled.pheno <- readRDS(file.path(data.results.dir, "Clinical_Phenotypes_Adjusted.RDS")) #Generated by 1a.Trait_Selection.Rmd
```
...
for(g in 1:length(gene.lists)){
    ...
    write.table(gene.lists[[g]], here("Results", "Gene_Sets", file.name),
