...
for(tx in 1:length(tissue.names)){
    ...
    a <- barplot(extreme.enrich, 
        ...
        xlim = c(min(path_enrich), max(path_enrich)), name = NA, col = tissue.cols[tx])
...
plot_kegg_picture <- function(search.term, quote.level = "####"){
    path.locale <- grep(search.term, all.kegg[[2]][,2], fixed = TRUE)
    path.id <- all.kegg[[2]][path.locale,][1]
    path.name <- all.kegg[[2]][path.locale,][2]
...
```{r oxphos}
kegg.term = "Oxidative phosphorylation"

plot_kegg_picture(kegg.term)
```
...
for(tx in 1:length(tissue.names)){
    ...
    a <- barplot(extreme.enrich, 
        horiz = TRUE, las = 2, main = tissue.names[tx], 
        xlim = c(min(go.enrich.nes), max(go.enrich.nes)), names = NA, col = tissue.cols[tx])
