...
write.csv(factor.covar, file.path(data.results.dir, "Covar.csv"))

group.def <- c("Sex", "Diet", "Treatment") #use these factors to define groups
groups <- unique(factor.covar[,group.def])
write.csv(groups, file.path(data.results.dir, "Groups.csv"), quote = FALSE)
...
saveRDS(expr, file.path(data.results.dir, "Expression.by.Tissue.RDS"))

#also save expression adjusted for all covariates
saveRDS(adj.expr, file.path(data.results.dir, "Expression.by.Tissue.Adjusted.RDS"))
...
write.table(mouse.weight.val, file.path(data.results.dir, "Weight.Summarized.csv"), 
...
write.csv(chem.mat, file.path(data.results.dir, "Blood.Chemistries.Summarized.csv"))
```
