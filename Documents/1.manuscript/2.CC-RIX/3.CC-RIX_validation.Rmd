...
if(is.na(use.cc.imputed.transcriptome)){
    use.cc.imputed.transcriptome = FALSE #set this to TRUE to do the equivalent of genomic prediction
}
...
if(plot.individuals){
    ...
    for(i in 1:length(tissue.expr)){
        ...
        text(x = -2, y = 1.75, labels = bquote(R^2 == .(stats[1])), adj = 0, cex = 1.2)
        text(x = -2, y = 1.25, labels = paste0("p = ", signif(stats[2], 2)), adj = 0, cex = 1.2)
    }
    dev.off()
}
