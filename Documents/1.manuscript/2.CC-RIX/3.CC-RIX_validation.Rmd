...
genoprobs <- readRDS(here("Results", "CC-RIX", "Genotypes", "Genoprobs.RDS")) #created by 3.5.CC-RIX_genotypes.Rmd
...
for(i in 1:length(tissue.expr)){
    ...
    pheno.cor <- t(sapply(1:ncol(adj.pheno), 
        ...
        main = colnames(adj.pheno)[x], xlab = "Compostite Expression")))
...
tissue.cols <- c("orange", "tan", "brown") #colors that are not related to the DO/CC colors
par(mar = c(8, 4, 4, 4))
