...
for(i in 1:length(tissue.expr)){
    ...
    #if we are using imputed gene expression, use strain means
    if(imp.text == "imputed"){
        u_strain <- unique(cc_strains)
        u_strain_idx <- lapply(u_strain, function(x) which(cc_strains == x))
        names(u_strain_idx) <- u_strain
        strain.mge <- sapply(u_strain_idx, function(x) mean(mean.mge[x])) #these are all the same for a single strain, but the mean works
        norm.pheno <- apply(adj.pheno, 2, rankZ)
        strain.pheno <- t(sapply(u_strain_idx, function(x) colMeans(norm.pheno[x,], na.rm = TRUE)))
        strain.error <- t(sapply(u_strain_idx, function(x) apply(norm.pheno[x,], 2, function(y) sd(y, na.rm = TRUE))))

        ymin <- min(strain.pheno - strain.error)
        ymax <- max(strain.pheno + strain.error)
        ...
        for(ph in 1:ncol(strain.pheno)){
            pheno.cor[ph,] <- plot.with.model(strain.mge, strain.pheno[,ph], 
                ...
                main = colnames(strain.pheno)[ph], xlab = "Predicted Metabolic Index",
                ylim = c(ymin, ymax))
            segments(x0 = strain.mge, y0 = strain.pheno[,ph]+strain.error[,ph], 
                y1 = strain.pheno[,ph]-strain.error[,ph])
        }
    }else{
...
```{r pop_traits_fig, warning = FALSE, eval = FALSE}
