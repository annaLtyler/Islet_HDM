...
boxplot(status_list[mean.order], xlab = "", ylab = "", 
    main = "", names = NA)
mtext("score", side = 2, line = 2.5, cex = 2)
abline(h = 0)
par(xpd = NA)
mtext("diabetic vs. non-diabetic", side = 3, line = 1, cex = fig.main.cex)
text(x = c(1,2), y = rep(-4.5e-4, 2), 
    labels = str_to_lower(gsub("_", "\n", names(status_list[mean.order]))), 
    cex = fig.label.cex)
plot.text(paste("p =", signif(p, 2)), rel.x = 0.98, rel.y = 0.07, add = TRUE, 
    adj = 1, cex = 2)
dev.off()
```
...
text(x = c(1,2), y = rep(-0.0025, 2), 
    labels = str_to_lower(gsub("visceral", "", gsub("_", "\n", names(status_list[mean.order])))), 
    cex = fig.label.cex)
plot.text(paste("p =", signif(p, 2)), rel.x = 0.98, rel.y = 0.07, add = TRUE, 
    adj = 1, cex = 2)
dev.off()
