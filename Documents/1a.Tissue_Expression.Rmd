...
if(use.kinship){
  ...
  tissue.grid.file <- file.path(results.dir, "Transcripts_by_Tissue_no_kin.RDS")
}
...
I'm curious to see examples of genes with significant and 
non-significant tissue-specific eQTL and high and low 
LOD scores.
...
CAST and NZO appear to have strong negative effects on 
transcription, but only in liver. B6 has a strong negative
effect, but only in adipose.
...
The effects of the CAST allele across different tissues
are shown below, and I'm not convinced that there is a
tissue-specific effect. All the effects are fairly weak.

```{r cast_effect, fig.width = 9, fig.heigh = 6}
expr_across_tissues(int.examples[8], "CAST")
