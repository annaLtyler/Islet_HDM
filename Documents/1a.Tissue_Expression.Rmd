...
for(tx in 1:length(tissue.names)){
    ...
    if(file.exists(distal.herit.file) && !overwrite.results){
      ...
      start.idx <- max(which(!is.na(herit.resid))) + 1
    }
  ...
  if(start.idx < length(transcript.id)){
    ...
    herit.resid <- rep(NA, length = length(transcript.id))
    names(herit.resid) <- transcript.id
    ...
    for(tr in start.idx:length(transcript.id)){
      ...
      model <- fit1(marker.geno, rankZ(adj.expr[[tx]][,tr]))
      herit.resid[tr] <- est_herit(model$resid, Kg)
      if(tr %% save.every == 0){
