...
We looked at the relative contribution of local
genotype vs. an interaction getween genotype and
tissue. The following histogram shows the difference
between the contribution from the interaction and
the contribution from the local genotype. 

Negative values indicate transcripts for which 
the local genotype contributed more than the interaction
to variation in gene expression. Positive values
indicate transcripts for which the interaction 
contributed more than the local genotype.

The dot plot shows the maximum local LOD scores
for these genes compared to the difference in 
contributions from the local genotype and interaction.

The transcripts with more contribution from the interaction
tend to have lower LOD scores, but still have a range, particularly 
for those transcripts for which the interaction and genotype
contribute fairly evenly. 

```{r rel_effect}
int.diff <- prop.effect[,"Interaction"] - rel.effect.mat[,"Genotype"]
par(mfrow = c(1,2))
hist(int.diff, breaks = 100, xlab = "Difference", 
  main = "Interaction - Genotype")
abline(v = 0, col = "red")
plot(int.diff, max.lod, xlab = "Difference", ylab = "Maximum LOD Score",
  pch = 16)
abline(v = 0, col = "red")
```
...
trans.id <- names(sorted.int.diff)[1] #small interaction
plot_tissue_interaction(trans.id, FALSE, text.gap = 3, text.shift = 2)

#plot_tissue_interaction(names(tissue.lower)[2], FALSE)
```
...
trans.id <- names(sorted.high.int)[1] #large interaction
...
trans.id <- names(int.stronger)[4]
plot_tissue_interaction(trans.id, FALSE)
expr_across_alleles(trans.id, "Heart")
...
expr_across_alleles(transcript.id = trans.id, "Liver")
```
