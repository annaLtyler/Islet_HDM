...
plot_tissue_interaction <- function(transcript.id, use.kinship = FALSE){
  ...
  if(length(has.expr) > 1){
    ...
    boxplot(expr.mat, main = "Expression by tissue", col = tissue.cols[has.expr], 
      las = 2)
    barplot(tissue.lod, main = "LOD score by tissue", col = tissue.cols[has.expr], 
      las = 2)
...
The following plot shows an example of a transcript that
does not have a significant tissue-specific eQTL. All 
alleles have similar effects across all tissues.

```{r no_int, fig.width = 7, fig.height = 7}
no.int <- rownames(p.mat)[order(p.mat[,"Tissue_by_Genotype"], decreasing = TRUE)][1:10]
plot_tissue_interaction(no.int[2], FALSE)
```
...
max.lod <- apply(tissue.lod, 1, function(x) if(!all(is.na(x))){max(x, na.rm = TRUE)}else{NA})
plot(max.lod, -log10(p.mat[,"Tissue_by_Genotype"]), xlab = "LOD", ylab = "-log Interaction p value")
```

```{r example_grid, eval = FALSE}
...
int.examples <- intersect(which(max.lod < 50), which(-log10(p.mat[,"Tissue_by_Genotype"]) > 3))
plot_tissue_interaction(rownames(p.mat)[int.examples[7]], FALSE)
