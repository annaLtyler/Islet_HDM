...
distal.herit.file <- here("Results", "Transcriptome", "Distal_Heritability.RDS")

if(!file.exists(distal.herit.file)){
  distal.herit <- vector(mode = "list", length = length(tissue.names))
  names(distal.herit) <- tissue.names
  ...
  for(tx in 1:length(tissue.names)){
    ...
    for(tr in 1:length(transcript.id)){
    ...
    distal.herit[[tx]] <- herit.resid
  }
  saveRDS(distal.herit, distal.herit.file)
}
```