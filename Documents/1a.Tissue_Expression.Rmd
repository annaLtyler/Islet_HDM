...
all.packages <- c("igraph", "pheatmap", "gprofiler2", "qtl2", "stringr",
  "wordcloud", "wordcloud2", "hexbin", "regress")
...
#tissue.cols <- categorical_pal(length(tissue.names)) #these overlap a lot with DO/CC colors
#tissue.cols <- brewer.pal(length(tissue.names), "Set2")
...
if(file.exists(int.p.file)){
  ...
  relative.effect <- matrix(NA, nrow = length(all.tx), ncol = 3)
  rownames(relative.effect) <- all.tx
  colnames(relative.effect) <- c("Tissue", "Genotype", "Interaction")
...
if(start.at < length(all.tx)){
  ...
  #for(tx.idx in 1:200){
    ...
    if(length(has.expr) > 1){
      ...
      all.results <- coef(test)
      tissue.results <- grep("tissue", names(all.results))
      geno.results <- grep("geno", names(all.results))
      just.tissue.results <- setdiff(tissue.results, geno.results)
      just.geno.results <- setdiff(geno.results, tissue.results)
      both.results <- intersect(geno.results, tissue.results)
      #boxplot(list("tissue" = all.results[just.tissue.results], "genotype" = all.results[just.geno.results], "interaction" = all.results[both.results]))
      tissue.effect <- sqrt(mean(all.results[just.tissue.results]^2, na.rm = TRUE))
      geno.effect <- sqrt(mean(all.results[just.geno.results]^2, na.rm = TRUE))
      int.effect <- sqrt(mean(all.results[both.results]^2, na.rm = TRUE))
      relative.effect[tx.idx,] <- c(tissue.effect, geno.effect, int.effect)
      #barplot(relative.effect[tx.idx,])
...
```{r ptissue}
fdr.mat <- apply(p.mat, 2, function(x) p.adjust(x, "holm"))
hist(-log10(fdr.mat[,1]), breaks = 100, 
  main = "Tissue Main Effect -log10(p value)",
  xlab = "-log10(p value)")
...
```{r pgeno}
hist(-log10(fdr.mat[,2]), breaks = 100, 
  main = "Genotype Main Effect -log10(p value)",
  xlab = "-log10(p value)")
...
```{r pint}
hist(-log10(fdr.mat[,3]), breaks = 100, 
  main = "Tissue by Genotype Interaction -log10(p value)",
  xlab = "-log10(p value)")
```
...
plot_tissue_interaction <- function(transcript.id, use.kinship = FALSE){
  ...
  if(length(has.expr) > 1){
    ...
    int.p <- signif(anova(test)[3,"Pr(>F)"], 2)
    #anova(test)  
    ...
    barplot_with_num(round(tissue.lod), main = "LOD score by tissue", 
      col = tissue.cols[has.expr], las = 2)
    ...
    text(x = colMeans(a), y = rep(min(all.coef[1:8,])-bar.range*0.05, 8), 
      labels = names(CCcolors), col = CCcolors, font = 2)
    par(xpd = FALSE)
  ...
  }else{ #end case for having expression in at least two tissues
    plot.text("Not expressed in multiple tissues.")
  }
}
...
expr_across_alleles <- function(transcript.id, tissue.name){
  ...
  tissue.idx <- lapply(tissue.tx, function(x) which(x == all.tx[tx.idx]))
  has.expr <- which(sapply(tissue.idx, length) > 0)
  selected <- which(names(has.expr) == tissue.name)
  
  if(length(selected) == 1){

    tx.expr <- ind.matched.expr[[has.expr[selected]]][,tissue.idx[[has.expr[selected]]],drop=FALSE]
    colnames(tx.expr) <- names(ind.matched.expr)[has.expr[selected]]
    ...
    for(a in 1:ncol(nearest.geno)){
      plot.with.model(nearest.geno[rownames(tx.expr),a], tx.expr, 
...
expr_across_tissues <- function(transcript.id, allele.name){
  ...
  if(length(tissue.idx) > 1){
    ...
    layout(get.layout.mat(length(has.expr)))
    for(tx in 1:ncol(tx.expr)){
      plot.with.model(nearest.geno[rownames(tx.expr),allele.idx], tx.expr[,tx], 
        xlab = "Genotype", ylab = paste(gene.name, "expression"),
        main = tissue.names[has.expr[tx]], col = tissue.cols[has.expr[tx]],
        report = "cor.test")
    }  
...
with strongly tissue-specific eQTL. To identify the
interaction look for different bar heights in each
group of effects. Each group represents the effects
of a single allele on transcript levels across all 
tissues. If there is a tissue-specific effect, the
bars in a single group will have very different 
heights.

In the following example, the PWK allele has 
a strong positive effect on Mybl expression in
liver, but neglible to modestly negative effects
on Mybls expression in other tissues.


```{r strong_int, fig.width = 7, fig.height = 7}
strong.int <- rownames(p.mat)[which(p.mat[,"Tissue_by_Genotype"] == min(p.mat[,"Tissue_by_Genotype"], na.rm = TRUE))]
plot_tissue_interaction(strong.int[3], FALSE)
```

We can see this when we plot the PWK effect across
tissues. The positive effect in liver is in stark
contrast to the effects in the other tissues.

```{r pwk_int, fig.width = 7, fig.height = 5}
expr_across_tissues(strong.int[3], "PWK")
```

There is a very positive correlation between LOD score and 
interaction p value, which makes sense. We can't see 
tissue-specific eQTL for transcripts with low local LOD 
scores.
...
plot(max.lod, -log10(p.mat[,"Tissue_by_Genotype"]), xlab = "LOD", ylab = "-log Interaction p value")
```

## Examples

Below we look through examples of genes with significant 
and non-significant tissue-specific eQTL and high and low 
LOD scores.

### Strong eQTL, no interaction

The plot below shows an example of a transcript with a high
LOD score, but no tissue-specific eQTL. Rspl9 has a significant
eQTL in all tissues, and all allele coefficients are the same
across all tissues.

```{r high_lod_no_int, fig.height = 7, fig.width = 7}
#examples of genes with high LOD scores but no interaction
non.int.examples <- intersect(which(max.lod > 100), which(-log10(p.mat[,"Tissue_by_Genotype"]) < 3))
plot_tissue_interaction(rownames(p.mat)[non.int.examples[1]], FALSE)
```


### Strong eQTL, strong interaction

These don't all seem sound either. The following
example shows data for a transcript with a strong
eQTL, and a highly significant interaction score. 
However, the allele effects all look pretty similar.
Even though this interaction is highly significant,
I'm not convinced that it's real.

```{r high_lod_high_int, fig.height = 7, fig.width = 7}
#examples of genes with high LOD scores but no interaction
int.examples <- rownames(p.mat)[intersect(which(max.lod > 100), which(-log10(p.mat[,"Tissue_by_Genotype"]) > 50))]
plot_tissue_interaction(int.examples[1], FALSE)
expr_across_tissues(int.examples[1], "WSB")
```

In contrast, the next example from this category, does 
look as if it has a genuine interaction. The WSB allele
has a strong positive effect on expression only in islet
and liver.

```{r good_example, fig.width = 7, fig.height = 7}
plot_tissue_interaction(int.examples[2], FALSE)
```

The expression plots across tissues convincingly
support this interaction.

```{r wsb_effect, fig.height = 6, fig.width = 9}
expr_across_tissues(int.examples[2], "WSB")
```

### No eQTL, strong interaction

The plot below shows an example of a transcript with 
a weak eQTL, but a strongly significant tissue interaction.

PWK has a strong positive effect, but only in islet.

```{r no_eqtl_strong_int, fig.height = 7, fig.width = 7}
#examples of genes with low LOD scores and significant interactions
#only the extremely significant values have clear effects.
#many interaction effects are messy looking
int.examples <- rownames(p.mat)[intersect(which(is.na(max.lod)), which(-log10(fdr.mat[,"Tissue_by_Genotype"]) > 3))]
plot_tissue_interaction(int.examples[8], FALSE)
```

This is supported by individual expression plots.

```{r cast_effect, fig.width = 9, fig.height = 6}
expr_across_tissues(int.examples[8], "PWK")
#expr_across_tissues(int.examples[8], "B6")
```
...
```{r, eval = FALSE}
#Wfdc21
plot_tissue_interaction("ENSMUSG00000051748", FALSE)
expr_across_tissues(transcript.id = "ENSMUSG00000051748", "WSB")
expr_across_alleles(transcript.id = "ENSMUSG00000051748", "Liver")
```