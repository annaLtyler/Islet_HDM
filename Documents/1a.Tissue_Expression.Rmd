...
plot_tissue_interaction <- function(transcript.id, use.kinship = FALSE){
  ...
  if(length(has.expr) > 1){
    ...
    par(mar = c(4,4,4,4))
    boxplot(expr.mat, main = "Expression by tissue", col = tissue.cols, las = 2)
    barplot(tissue.lod, main = "LOD score by tissue", col = tissue.cols, las = 2)
    ...
    mtext(gene.name, side = 3, outer = TRUE, line = -1.5, font = 4, cex = 1.5)
...
The following plot shows an example of a transcript with a tissue-specific
eQTL and a transcript without a tissue-specific eQTL.

```{r no_int, fig.width = 7, fig.height = 7}
no.int <- rownames(p.mat)[which.max(p.mat[,"Tissue_by_Genotype"])]
plot_tissue_interaction(no.int, FALSE)
```


```{r strong_int}
strong.int <- rownames(p.mat)[which.min(p.mat[,"Tissue_by_Genotype"])]
plot_tissue_interaction(strong.int, FALSE)
```