...
```{r adjust}
adj.expr.file <- here("Data", "DO", "Tissue_Expression_Adjusted.RDS")
...
```{r, eval = FALSE}
clin.pheno <- readRDS(here("Data", "DO", "Clinical_Phenotypes_Adjusted.RDS"))
...
## Distal variance explained

We would like to look at the local vs. the distal components 
of MGE for the population. We can't do this for individuals,
but we can do it for the population. For each transcript, we
fit the local marker to the transcript levels. Then we calculate
the heritability of the residuals. We multiply this number by 
the transcript loading to get the distal MGE.

We can compare: 
  * the magnituge of MGE across local and distal components
  * enrichments of the the top and bottom scoring genes in both
  * Do genes with high distal MGE localize in distal eQTL hotspots?

```{r distal_variance_explained}

save.every = 100

for(tx in 1:length(tissue.names)){
    start.idx <- 1
    distal.herit.file <- here("Results", "Transcriptomes", 
      paste0("Distal_Heritability_", tissue.names[tx], ".RDS"))
    transcript.id <- colnames(adj.expr[[tx]])

    local.imp.file <- here("Results", "Transcriptomes", paste0("Local_Imputation_", tissue.names[tx], ".RDS"))
...
#start where we left off, or at the beginning if there are no data
  ...
  if(start.idx < length(transcript.id)){
    sink(here("Results", "Transcriptomes", paste0("progress_", tissue.names[tx], ".txt")))
    ...
    transcript.idx <- match(transcript.id, tissue.data[[tx]]$annot.mrna$gene.id) 
    nearest.marker <- tissue.data[[tx]]$annot.mrna$nearest.marker.id

    #mean-center all expr
    centered.expr <- apply(adj.expr[[tx]], 2, function(x) x - mean(x, na.rm = TRUE))
    #scale all variance to 1
    scaled.expr <- apply(centered.expr, 2, function(x) x/sd(x, na.rm = TRUE))
  
    #start where we left off, or at the beginning, if that is warranted
    for(tr in start.idx:length(transcript.id)){
      print(tr)
      if(nearest.marker[tr] == ""){next()}
      marker.geno <- pull_genoprobpos(genoprobs, marker = nearest.marker[tr])
      trans.expr <- scaled.expr[,transcript.id[tr]]
      ...
      #without kinship
      #model.no.kin <- fit1(marker.geno, trans.expr)
      #imp.local <- model.no.kin$fitted
      #distal.resid <- model.no.kin$resid
      #result <- c(var(imp.local), var(distal.resid), est_herit(distal.resid, Kg))
      #herit.resid[tr,] <- result

      #straight linear model
      #model <- lm(trans.expr~marker.geno[names(trans.expr),])
      #imp.local <- predict(model)
      #distal.resid <- residuals(model)
      #result <- c(var(imp.local), var(distal.resid), est_herit(distal.resid, Kg))
      #herit.resid[tr,] <- result
