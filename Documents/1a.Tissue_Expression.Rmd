...
for(tx in 1:length(tissue.names)){
  ...
  if(start.idx < length(transcript.id)){
    ...
    if(!file.exists(distal.herit.file) || overwrite.results){
      herit.resid <- matrix(NA, ncol = 2, nrow = length(transcript.id))
      rownames(herit.resid) <- transcript.id
      colnames(herit.resid) <- c("residual_variance", "residual_heritability")
    }
    ...
    for(tr in start.idx:length(transcript.id)){
      ...
      model <- fit1(marker.geno, rankZ(adj.expr[[tx]][,tr]), kinship = Kg)
      herit.resid[tr,1] <- var(model$resid)
      herit.resid[tr,2] <- est_herit(model$resid, Kg)
      if(tr %% save.every == 0){
