...
random effect. This procedure is performed in 1a.Tissue_Expression.Rmd. 
...
We are implementing that here in replacement of the Bayesian 
imputations.
...
if(use.local.imputation){
    ...
    imp.files <- sapply(tissue.names, function(x) paste0("Distal_Heritability_", x, ".RDS"))
    tissue.imp.local <- lapply(imp.files, function(x) readRDS(here("Results", "Transcriptomes", x)))
    tissue.imp.gen <- lapply(tissue.imp.local)
}else{
...
if(mediation.text == "complete_mediation"){
    ...
    for(tx in 1:length(tissue.names)){
        ...
        png(file.path(results.dir, paste0("MGE_Binarized_Distribution_", tissue.names[tx], "_", mge.name, ".png")), width = 5, height = 5, units = "in", res = 200)
...
```{r loading_motifs}
big.thresh = 0.2
thresh.loading <- toa.loading.mat
...
```{r mge_coherence}
hist(as.vector(toa.mge.mat))
abline(v = c(-0.05, 0.05), col = "red")

big.thresh = 0.05
thresh.loading <- toa.mge.mat
thresh.loading[which(abs(thresh.loading) < big.thresh)] <- 0
#pheatmap(thresh.loading)
...
```{r inbred_mge_dist, results = "asis"}
if(mediation.text == "complete_mediation" && use.local.imputation){
    for(tx in 1:length(tissue.names)){
        cat("###", tissue.names[tx], "\n")
...
```{r risk_protection_enrich}
if(mediation.text == "complete_mediation" && use.local.imputation){

    inbred.top.enrichment.file <- file.path(results.dir, "Inbred_High_MGE_Enrichments.RDS")
    inbred.bottom.enrichment.file <- file.path(results.dir, "Inbred_Low_MGE_Enrichments.RDS")
    num.top.tx <- 150
...
```{r plot_founder_enrichments, results = "asis", fig.width = 7, fig.height = 9}
if(mediation.text == "complete_mediation" && use.local.imputation){
...
if(mediation.text == "complete_mediation" && use.local.imputation){
    ...
    all.tissue.do <- sapply(tissue.mge, function(x) colMeans(x[,common.ind]))
...
```{r distal_var}
distal.var.files <- list.files(here("Results", "Transcriptomes"), pattern = "Distal_Heritability", 
    full.names = TRUE) #created by 1a.Tissue_Expression.Rmd
distal.var <- lapply(distal.var.files, readRDS)
names(distal.var) <- tissue.names
```
...
for many of the transncripts, although the bulk of the distribution
is below 0.5.
...
Yes. Why?
These numbers are positively correlated suggesting that the 
more heritable variance is explained by local genotype,
the more is also explained by distal variation. If these
two numbers are correlated, we'd want them to be negatively
correlated: the more variance is explained by local genotype,
the less is explained by distal factors, proportionately. These
numbers should also sum to no more than 1, but many of them doo.

```{r compare_to_local}
local.imp <- readRDS(here("Data", "imputed", paste0("Adjusted_Expression_DO_imputed_local.RDS")))
local.var <- lapply(local.imp, function(x) apply(x, 2, var))

par(mfrow = c(2,2))
for(tx in 1:length(tissue.names)){
    common.transcripts <- intersect(names(local.var[[tx]]), rownames(distal.var[[tx]]))
    plot(local.var[[tx]][common.transcripts], distal.var[[tx]][common.transcripts,"local_var"],
        xlab = "Matt's estimate", ylab = "Local model prediction")
    abline(0,1)
}
...
for(tx in 1:length(tissue.names)){
    #common.tx <- intersect(names(dist.var.exp[[tx]]), names(local.var[[tx]]))
    #plot.with.model(local.var[[tx]][common.tx], dist.var.exp[[tx]][common.tx],
    #    xlab = "Local variance explained", ylab = "Distal variance explained",
    #    main = tissue.names[tx])

    common.tx <- intersect(rownames(distal.var[[tx]]), names(dist.var.exp[[tx]]))

    plot.with.model(distal.var[[tx]][common.tx,"local_var"], dist.var.exp[[tx]][common.tx],
        xlab = "Local variance explained", ylab = "Distal variance explained",
        main = tissue.names[tx])

    total.sum[[tx]] <- distal.var[[tx]][common.tx,"local_var"] + dist.var.exp[[tx]][common.tx]


}

par(mfrow = c(1,1))
boxplot(total.sum);abline(h = 1)
```

What is correlated about these numbers? Is it that kinship piece isn't
being completely removed when we fit the linear model? What is fit1 
returning? Is it the variance explained by both the kinship and the
local bit? Should we correct for kinship before we fit the model?
Should we not include kinship in the model?

How do we figure out what is being replicated in the two models?
Look for correlation between the residuals and the original 
transcript?

## Distal hotspots
We want to see if high MGE genes are at all related to 
distal eQTL hotspots identified in 1a.Tissue_Expression.Rmd.
To examine this, we will look at MGE of genes based on their
...
for(tx in 1:length(tissue.names)){
    ...
    #collect MGE data for this tissue
    transcript.id <- names(pop.MGE[[tx]])
    transcript.idx <- match(transcript.id, gene.tables[[tx]]$gene.id)
    transcript.position <- gene.tables[[tx]][transcript.idx,]

    #collect distal eQTL data for this tissue
    distal.eqtl.markers <- distal.coef[[tx]]$marker.id
    distal.split.markers <- strsplit(distal.eqtl.markers, "_")
    distal.marker.chr <- sapply(distal.split.markers, function(x) x[1])
    distal.marker.pos <- as.numeric(sapply(distal.split.markers, function(x) x[2]))/1e6 #convert to Mb

    #collect local eQTL data for this tissue
    local.eqtl.markers <- local.coef[[tx]]$marker.id
    local.split.markers <- strsplit(local.eqtl.markers, "_")
    local.marker.chr <- sapply(local.split.markers, function(x) x[1])
    local.marker.pos <- as.numeric(sapply(local.split.markers, function(x) x[2]))/1e6 #convert to Mb

    all.marker.mge <- vector(mode = "list", length = 20)
    names(all.marker.mge) <- c(1:19, "X")

    for(ch in c(1:19, "X")){

        #get gene positions and MGE for all transcripts on the chromosome
        chr.idx <- which(transcript.position$chr == ch)
        chr.pos <- transcript.position$middle[chr.idx]
        pos.order <- order(chr.pos, decreasing = FALSE)
        all.marker.mge[[ch]] <- cbind(chr.pos[pos.order], pop.MGE[[tx]][chr.pos[pos.order]])

        #get marker positions for local eQTL
        local.marker.chr.idx <- which(local.marker.chr == ch)
        local.marker.chr.pos <- local.marker.pos[local.marker.chr.idx]
        local.marker.order <- order(local.marker.chr.pos, decreasing = FALSE)

        #get marker positions for distal eQTL
        distal.marker.chr.idx <- which(distal.marker.chr == ch)
        distal.marker.chr.pos <- distal.marker.pos[distal.marker.chr.idx]
        distal.marker.order <- order(distal.marker.chr.pos, decreasing = FALSE)

        max.chr.mb <- max(c(chr.pos, local.marker.chr.pos, distal.marker.chr.pos))
        max.x <- ceiling(max.chr.mb/50)*50
        xlim <- c(0, max.x)

        #plot MGE for each transcript at the position that it is
        #encoded on the genome
        par(mfrow = c(3,1), mar = c(0, 4, 4, 4))
        plot(chr.pos[pos.order], abs(pop.MGE[[tx]][chr.pos[pos.order]]), type = "h",
            ylab = "population MGE", main = paste(tissue.names[tx], "Chr", ch),
            xlim = xlim, axes = FALSE)
        axis(2)
        abline(h = 0)

        #plot local eQTL LOD scores for each marker along the chromosome
        par(mar = c(1, 4, 1, 4))
        max.lod <- max(local.coef[[tx]]$lod[local.marker.chr.idx[local.marker.order]])
        max.y <- ceiling(max.lod/10)*10
        plot(local.marker.chr.pos[local.marker.order], 
            local.coef[[tx]]$lod[local.marker.chr.idx[local.marker.order]], 
            type = "h", col = "#e34a33", xlim = xlim, 
            xlab = paste("Chromosome", ch, "Position (Mb)"),
            ylab = "Local LOD", axes = FALSE, ylim = c(0, max.y))
        axis(2)
        abline(h = 0)

        #plot distal eQTL LOD scores for each marker along the chromosome
        par(mar = c(4, 4, 1, 4))
        plot(distal.marker.chr.pos[distal.marker.order], 
            distal.coef[[tx]]$lod[distal.marker.chr.pos[distal.marker.order]], 
            type = "h", col = "#2b8cbe", xlim = xlim, 
            xlab = paste("Chromosome", ch, "Position (Mb)"),
            ylab = "Distal LOD", axes = FALSE)
        axis(1);axis(2)
