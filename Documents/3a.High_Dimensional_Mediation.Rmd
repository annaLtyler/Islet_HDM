...
if(use.local.imputation){
    ...
    #use simple model instead. These imputed transcripts are the fitted
    #values from fit1 with the nearest marker and a kinship correction
    imp.files <- sapply(tissue.names, function(x) paste0("Local_Imputation_", x, ".RDS"))
    tissue.imp.gen <- lapply(imp.files, function(x) readRDS(here("Results", "Transcriptomes", x)))
}else{
...
pop.local.mge.file <- file.path(results.dir, paste0("Population_Local_MGE_", mge.name, ".RDS"))
...
```{r enrichment}
enrich.file <- file.path(results.dir, "Top_Bottom_Loading_Enrichment.RDS")
...
if(mediation.text == "complete_mediation"){
    ...
    for(tx in 1:length(tissue.names)){
        ...
        #hist(pop.local.MGE[[tx]], breaks = 100)
...
toa.mge.mat <- cluster_values(pop.local.MGE, z.thresh = 3, k.seq = 3:20, 
    pdf.filename = "TOA_Local_MGE.pdf")
...
pop.distal.mge.file <- file.path(results.dir, paste0("Population_Distal_MGE_", mge.name, ".RDS"))
...
toa.distal.mge.mat <- cluster_values(pop.distal.MGE, z.thresh = 3, k.seq = 3:20, 
    pdf.filename = "TOA_Distal_MGE.pdf")
```
