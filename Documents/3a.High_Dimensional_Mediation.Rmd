...
```{r perm}
perm.file <- file.path(results.dir, "Permuted_Stats.RDS") #holds permuted path coefficients and partial correlations between components
null.mediator.file <- file.path(results.dir, "Permuted_Mediator_Scores.RDS") #holds permuted mediator scores, which can be used to calculate null transcript loadings and MGE
...
saveRDS(pop.MGE, file.path(results.dir, paste0("Population_MGE_Loadings_", mge.name, ".RDS")))
...
for(tx in 1:length(pop.MGE)){
    plot(density(null_MGE[[tx]]), type = "l", col = "gray", lwd = 3,
        xlim = c(xmin, xmax), ylim = c(0, ymax), xlab = "Loading",
        main = names(transcript_loadings[tx]))
    points(all.dens[[tx]], type = "l", col = tissue.cols[tx], lwd = 3)
    legend("topleft", col = c(tissue.cols[tx], "gray"), legend = c("observed", "null"), 
...
#pheatmap(toa.loading.mat, annotation_row = enrich.df, show_rownames = TRUE, cex = 0.7, cluster_cols = FALSE, annotation_color = ann_cols)
...
#pdf("~/Desktop/test.pdf", width = 5, height = 15)
#pheatmap(toa.mat, annotation_row = enrich.df, show_rownames = TRUE, cex = 0.7, cluster_cols = FALSE)
#pheatmap(toa.mat, show_rownames = TRUE, cex = 0.7, cluster_cols = FALSE)
#dev.off()
```
