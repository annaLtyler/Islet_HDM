...
tissue.data.file <- lapply(tissue.names, 
  function(x) get.files(here("Data", "DO"), want = x, 
    dont.want = c("Gene", "vivo", "Residual"), full.names = FALSE))
...
```{r pheno}
scaled.pheno <- readRDS(here("Data", "DO", "Clinical_Phenotypes_Adjusted.RDS"))
```
...
residual.expr.files <- sapply(tissue.names, function(x) here("Data", "DO", paste0(x, "_Residual.RDS")))
...
gene.deg <- readRDS(here("Results", "Transcription_Networks", "Transcript_Degree.RDS")) #calculated in 1a.Tissue_Expression.Rmd
