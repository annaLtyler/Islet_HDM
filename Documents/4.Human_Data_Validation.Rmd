...
expr.sample.id <- sapply(strsplit(colnames(adj.expr), "_"), function(x) x[1])
...
for(tx in 1:length(tissue.names)){
    ...
    test <- aov(tx.pred~ob.status)
    p <- anova(test)[,"Pr(>F)"][1]
    boxplot(tx.pred~ob.status, xlab = "", ylab = "Prediction", 
...
ob.status <- as.factor(sapply(strsplit(matched.covar[,"status"], ": "), function(x) x[2]))
...
Accession number: GSE200678

To understand the role of adipose tissue senescence in 
NAFLD/NASH, RNA sequencing was performed in the visceral 
adipose tissue of NAFLD and NASH pateints.

Comparative gene expression profiling analysis of RNA-seq 
data for visceral adipose tissue of NAFLD and NASH patients

```{r nafld}
# load counts table from GEO
accession <- "GSE200678"

num.counts <- as.matrix(read.delim(here("Data", "Human", paste0(accession, "_Kallisto_visceral_AT_tissue_count.txt")), row.names = 1))
gene.name <- rownames(num.counts)
...
#adjust for sex
covar.table <- read_seq_meta(accession, named.numbers = c("Pt_Label" = 29, "Status" = 40,
    "sex" = 41, "age" = 42, "bmi" = 43, "disease" = 44), color.by = "Status")

#adjust for covariates
sex <- dummy_covar(covar.table[,"sex",drop=FALSE])
age <- as.numeric(sapply(strsplit(covar.table[,"age"], ":"), function(x) x[2]))
adj.expr <- adjust(t(dat), cbind(sex, age))

#cbind(colnames(dat), covar.table[,1])
...
for(tx in 1:length(tissue.names)){
    ...
    pval <- anova(group.test)[,"Pr(>F)"][1]
    boxplot(tx.pred~as.factor(covar.table[,"Status"]), xlab = "", 
        ylab = "Predicted Phenotype", 
        main = paste(tissue.names[tx], "\np =", signif(pval, 2)))
    abline(h = 0)
...
```{r nafld_quant, fig.width = 12, fig.height = 9}
quant.col <- c("bmi")
...
for(tx in 1:length(tissue.names)){
    ...
    for(p in 1:length(quant.col)){
        trait <- as.numeric(sapply(strsplit(covar.table[,quant.col[p]], ":"), function(x) x[2]))
        plot.with.model(tx.pred, trait, xlab = "Prediction", ylab = quant.col[p], main = quant.col[p],
            report = "cor.test")
    }
