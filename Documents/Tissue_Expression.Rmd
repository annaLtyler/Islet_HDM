...
tissue.data <- lapply(tissue.names, 
  function(x) readRDS(here("Data", paste0(x, ".RDS"))))
names(tissue.data) <- tissue.names
load(here("Data", "QTLViewer_Geno_V10.Rdata"))
```
...
if(remove.pseudogenes){
  ...
  for(tx in 1:length(tissue.data)){
    description.file <- here("Data", paste0("Gene_Description_", tissue.names[tx], ".RDS"))
    if(!file.exists(description.file)){
      one.gene.info <- getBM(attributes = c("ensembl_gene_id", "description"), 
        ...
        mart = mus)
      saveRDS(one.gene.info, description.file)
    }else{
      one.gene.info <- readRDS(description.file)
    }
...
#plot decomposition
decomp.file <- here("Results", "Tissue.Decomposition.RDS")
...
local.coef.file <- here("Results", "eQTL_coef_local.RDS")
distal.coef.file <- here("Results", "eQTL_coef_distal.RDS")
...
local.eqtl.binary.file <- here("Results", "Shared_eQTL_Local_Binary.RDS")
local.eqtl.coef.file <- here("Results", "Shared_eQTL_Local_Coef.RDS")
...
#identify unique local eQTLs that are shared across tissues
distal.eqtl.binary.file <- here("Results", "Shared_eQTL_Distal_Binary.RDS")
distal.eqtl.coef.file <- here("Results", "Shared_eQTL_Distal_Coef.RDS")
...
local.eqtl.cor.file <- here("Results", "Correlation_Allele_Effects_Local.RDS")
...
distal.eqtl.cor.file <- here("Results", "Correlation_Allele_Effects_Distal.RDS")
...
local.expr.cor.file <- here("Results", "Correlation_Expression_Local.RDS")
...
distal.expr.cor.file <- here("Results", "Correlation_Expression_Distal.RDS")
...
distal.names <- apply(num.distal, 2, function(x) which(x > 0))
distal.enrich <- lapply(distal.names, function(x) gost(x, organism = "mmusculus"))
plot.enrichment.group(distal.enrich)

for(i in 1:length(tissue.names)){
  distal.idx <- match(names(distal.names[[i]]), tissue.data[[i]]$annot.mrna$gene.id)
  distal.table <- tissue.data[[i]]$annot.mrna[distal.idx,]
  write.table(distal.table, paste0("~/Desktop/distal_transcripts_", tissue.names[i], ".csv"),
