...
if(!is.null(adjust_before_fitting)){
    adjust.file <- here("Results", "High_Dim_Med_with_MGE", exp.name, adjust_before_fitting)
...
results.dir <- here("Results", "High_Dim_Med_with_MGE", exp.name)
...
### Local MGE vs. Degree {.tabset .tabset-fade .tabset-pills}
...
for(tx in 1:length(tissue.names)){
    ...
    plot.with.model(gene.deg[[tx]][common.tx], abs(pop.local.MGE[[tx]][common.tx]),
        xlab = "Degree", ylab = "|Local MGE|", main = tissue.names[tx], report = "cor.test")
...
for(tx in 1:length(tissue.names)){
    ...
    plot.with.model(gene.deg[[tx]][common.tx], abs(pop.distal.MGE[[tx]][common.tx]),
        xlab = "Degree", ylab = "|Distal MGE|", main = tissue.names[tx], report = "cor.test")
}
mtext("Degree vs. |Distal MGE|", side = 3, outer = TRUE, line = -1.5)
```

### Local MGE vs. Distal MGE
...
for(tx in 1:length(tissue.names)){
    ...
    plot.with.model(abs(pop.local.MGE[[tx]][common.tx]), 
        ...
        xlab = "Local MGE", ylab = "Distal MGE", main = tissue.names[tx], report = "cor.test")
...
## Predictions with transcription

To translate to humans we are creating phenotypic predictions
based on transcription values. We want to do the parallel
here in the mice to see what the distributions look like. Are 
they nudged in one direction or the other, or are they really 
pegged high or low?

```{r mouse_transcriptomic_prediction}
#scale expression across individuals as we do in humans
ind.scaled.expr <- lapply(adj.expr, function(x) apply(x, 1, scale))
for(tx in 1:length(ind.scaled.expr)){
    rownames(ind.scaled.expr[[tx]]) <- colnames(adj.expr[[tx]])
}

#multiply by loadings
par(mfrow = c(2,2))
for(tx in 1:length(tissue.names)){
    common.tx <- intersect(rownames(ind.scaled.expr[[tx]]), rownames(transcript_loadings[[tx]]))
    transcript.pred <- apply(ind.scaled.expr[[tx]][common.tx,], 2, function(x) x * transcript_loadings[[tx]][common.tx,])
    mean.pred <- colMeans(transcript.pred)
    pred.order <- order(mean.pred)    
    #boxplot(transcript.pred[,pred.order])
    
    common.ind <- intersect(colnames(ind.scaled.expr[[tx]]), rownames(scaled.pheno))
    plot.with.model(mean.pred[common.ind], scaled.pheno[common.ind,"weight_final"],
        xlab = "Prediction", ylab = "Final Weight", main = tissue.names[tx])
}
