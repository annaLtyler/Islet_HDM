...
sex.order <- rev(order(as.numeric(as.factor(covar[,1]))))
...
plot.with.model(num.pheno[sex.order,"food_ave"], num.pheno[sex.order,"weight_final"],
  ylab = "Weight (g)",
...
rownames(pc.mat) <- descriptive_names[,2]
...
gene.tables <- readRDS(file.path(data.results.dir, "Gene_Tables.RDS"))
...
tx.trait.cor.file <- here("Results", "eQTL", "Trait_Transcript_Cor.RDS")
if(!file.exists(tx.trait.cor.file)){
  ...
  for(tx in 1:length(tx.trait.cor)){
    common.ind <- intersect(rownames(adj.expr[[tx]]), rownames(scaled.pheno))
    tx.trait.cor[[tx]] <- apply(adj.expr[[tx]][common.ind,], 2, 
      function(x) max(abs(apply(scaled.pheno[common.ind,], 2, 
